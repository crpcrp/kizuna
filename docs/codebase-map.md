...
|---|---|---|
| Application shell and window | `App.tsx`, `components/WindowChrome.tsx`, `state/usePlaybackWindow.ts`, `state/useVideoWindowSizing.ts`, `state/appChrome.ts`, `state/windowSizing.ts`, `state/useVideoMargins.ts`, `state/useMiniPlayer.ts`, `state/usePlayerEvents.ts`, `shared/windowBounds.ts` | `index.ts`, `appLifecycle.ts`, `windowOptions.ts`, `windowPair.ts` (window creation, source-aware host/overlay coordination, lifecycle, fullscreen, and IPC target mapping) |
| Playback and player controls | `state/usePlaybackWindow.ts`, `state/useVideoWindowSizing.ts`, `components/BottomBar.tsx`, `components/MenuBar.tsx`, `state/playerAdapter.ts`, `state/playbackCommands.ts`, `state/keyActions.ts`, `state/perFileOffsets.ts`, `state/usePerFileRestore.ts`, `state/audioDevices.ts`, `shared/playerSettings.ts` | `playerBridge.ts`, `mpv/controller.ts`, `mpv/ipcClient.ts` |
...
| Jimaku subtitle discovery and export | `shared/jimaku.ts` (normalized entries, files, session results, prepared selections, export contracts, error results, credential status, and folder hints); `shared/jimakuMatching.ts` (local identity parsing and deterministic file ranking); `state/jimakuController.ts` (guarded search, preparation, replacement, revert, and remembered-title state); `state/useMediaSession.ts` and `components/menu/SubtitleMenu.tsx` (active-subtitle export command); `components/JimakuSubtitlesDialog.tsx` and `JimakuSubtitlesDialog.css` (presentation-only search, remembered-title controls, and file chooser); `components/NoJapaneseSubtitlePrompt.tsx` and its CSS (post-restore discovery prompt) | `services/jimaku/client.ts` (the fixed-origin, opt-in remote boundary), `services/jimaku/settings.ts` (encrypted credential and folder-hint owner), `services/jimaku/downloadStore.ts` (bounded direct/package storage, deduplication, cache lookup, and cleanup), `services/jimaku/archive.ts` (ZIP inspection and selected-member extraction), `services/jimaku/export.ts` (native Save As and byte-preserving destination write), `services/jimaku/service.ts` (main-owned sender-scoped search/download sessions and export validation); `jimakuSettingsBridge.ts`, `jimakuBridge.ts` |
...
| Tokenization | `state/useVocabularyCaches.ts`, `state/useVocabularyPipeline.ts`, `state/tokenization.ts`, `shared/token.ts`, `shared/mecab.ts`, `shared/textProjection.ts` (the display/analysis offset spaces every token offset lives in) | `mecabBridge.ts`, `services/mecab/` |
...
| Knowledge tracking and JLPT coverage/export | `state/useKnowledgeOptions.ts`, `state/useVocabularyCaches.ts`, subtitle components, `state/knowledgeActions.ts`, knowledge options, `shared/knowledge.ts`, `shared/jlptCoverage.ts`, `shared/jlptExport.ts` | `knowledgeBridge.ts` (IPC adapter), `services/knowledge/service.ts` (sync/settings/report/export orchestration), `services/knowledge/`, `services/jlpt/` |
...
| Azure translation | `components/options/SubtitlesTab.tsx`, subtitle and Game OCR translation surfaces, `state/integrationActions.ts`, `state/optionsData.ts`, `shared/translation.ts` | `translateBridge.ts`, `translationSettingsBridge.ts`, `services/translate/azureTranslate.ts`, `services/translate/translationSettings.ts` |
| Anki card creation | `state/useWordPopup.ts`, `state/useBulkMining.ts`, `state/useSubtitleReport.ts`, word and subtitle-report UI, `state/ankiMining.ts`, `state/bulkMiningController.ts`, `state/subtitleReportController.ts`, Anki options, `shared/anki.ts` | `ankiBridge.ts`, `services/anki/service.ts` (composition and ordinary card operations), `services/anki/jlptBackfillService.ts` (stateful JLPT preview/apply workflow) |
| Settings and appearance | `state/useOptionsDialog.ts`, `state/optionsMenuProps.ts`, `state/optionsData.ts`, `state/integrationActions.ts`, `components/OptionsMenu.tsx`, `components/options/SubtitlesTab.tsx` (subtitle, translation, and Jimaku credential controls), `state/playerState.ts`, `state/rendererSettings.ts` (which persisted fields the renderer syncs, and who owns the rest), `state/useSettingsLifecycle.ts`, `state/settingsPersistence.ts`, `state/useAppearance.ts`, `state/themeController.ts` | `playerSettingsBridge.ts`, `services/settings.ts`, `services/secrets.ts` |
| Game OCR (Windows, experimental) | `gameOcr.tsx` (the second renderer entry, loaded by `src/renderer/gameOcr.html`), `state/useGameOcrSession.ts`, `state/gameOcrBoxRegions.ts`, `state/gameOcrLayout.ts`, `state/gameOcrCaptureStream.ts` (retained capture streams), `state/gameOcrTextPipeline.ts`, `state/gameOcrSelection.ts`, `state/useGameOcrTranslation.ts`, `components/GameOcrFrame.tsx`, `components/GameOcrBoxes.tsx`, `components/GameOcrInteraction.tsx`, `state/useGameOcr.ts` and `components/options/GameOcrTab.tsx` (the player window's controls), `shared/ocr.ts`, `shared/gameOcr.ts`, `shared/gameOcrSettings.ts` | `gameOcrBridge.ts`, `services/gameOcr/controller.ts` (lifecycle facade: arm, stop, status), `services/gameOcr/shortcuts.ts` (accelerator ownership and repeat guard), `services/gameOcr/captureSession.ts` (sessions, recapture order, recognition), `services/gameOcr/captureTimings.ts` (per-capture stage timings and the latency log), `services/gameOcr/captureTarget.ts` (focused window or display fallback), `services/gameOcr/foregroundWindow.ts` (the Win32 boundary), `services/gameOcr/windowCapture.ts` (source ids, physical-to-logical geometry), `services/gameOcr/displayCapture.ts`, `services/gameOcr/frozenFrameController.ts` (the `GameOcrWindow` contract: readiness, capture waiters, discard/dismiss/close), `services/gameOcr/frozenFrameElectron.ts` (BrowserWindow options, construction and loading, content protection, display events, the unsupported-platform window), `services/gameOcr/frozenFrameIpc.ts` (Game OCR channel listeners and sender filtering), `services/gameOcr/runtime.ts`, `services/gameOcr/backgroundLifecycle.ts`, `services/gameOcr/tray.ts`, `services/ocr/ppOcrWorker.ts` (process lifecycle), `services/ocr/ppOcrProtocol.ts` (JSONL protocol), `resourcePaths.ts` (bundled payload) |
| Packaging and startup | `shared/appIdentity.json`, `shared/appIdentity.ts` | `appIdentity.ts`, `startupDecision.ts`, `resourcePaths.ts`, `startupProbe.ts`, `electron-builder.cjs`, `scripts/linuxPackaging.mjs`, `scripts/smoke-linux-package.mjs` |
| Application updates | `AppShell.tsx` (owns the one controller for every surface), `state/useUpdates.ts`, `state/updatesContext.tsx`, `state/updateController.ts`, `components/UpdateNotifications.tsx`, `components/UpdateDialog.tsx`, `shared/update.ts`, typed `preloadApi.ts` surface | `updateSupport.ts`, `electronUpdaterAdapter.ts`, `updaterErrors.ts`, `updateService.ts`, `updateBridge.ts`, lifecycle composition in `index.ts` |

Renderer paths in the table are relative to `src/renderer/src/`; shared and
main paths are relative to `src/`. Both HTML entry points live one level up:
`src/renderer/index.html` (→ `src/renderer/src/main.tsx`) and
`src/renderer/gameOcr.html` (→ `src/renderer/src/gameOcr.tsx`).

Each major subtree also carries a short scoped `AGENTS.md`
(`src/main/`, `src/renderer/`, `test/`, `scripts/`) with the rules that differ
there; the repository-wide rules stay in the root [AGENTS.md](../AGENTS.md).

Jimaku's remote boundary is main-owned and opt-in. `services/jimaku/client.ts`
is the only provider client and exposes no remote URLs to the renderer;
`services/jimaku/service.ts` turns explicit search and selection actions into
sender-scoped requests, while `services/jimaku/settings.ts` keeps the key and
folder hints local. Startup and media-open code restore the managed cache
without starting Jimaku requests.
...
`services/gameOcr/frozenFrameController.ts` separates that discard from the
close that only stopping, a display change, or a dead renderer performs. Work that
