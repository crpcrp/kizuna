...
| Knowledge tracking and JLPT coverage | `state/useKnowledgeOptions.ts`, `state/useVocabularyCaches.ts`, subtitle components, `state/knowledgeActions.ts`, knowledge options, `shared/knowledge.ts`, `shared/jlptCoverage.ts` | `knowledgeBridge.ts`, `services/knowledge/`, `services/jlpt/` |
