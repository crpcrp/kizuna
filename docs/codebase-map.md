...
| Jimaku subtitle discovery | `shared/jimaku.ts` (normalized entries, files, error results, and credential status); `shared/jimakuMatching.ts` (local identity parsing and deterministic file ranking) | `services/jimaku/client.ts`, `services/jimaku/settings.ts` (fixed-origin API client and encrypted credential owner), `services/jimaku/downloadStore.ts` (bounded direct-download storage and deduplication); `jimakuSettingsBridge.ts` |
