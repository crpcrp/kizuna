...
/**
 * The roaming-profile directory the app stores settings and databases in.
 * `appDataRoot` is Electron's `appData` path (`%APPDATA%` on Windows).
 *
 ...
 */
export function userDataDir(appDataRoot: string): string {
  return join(appDataRoot, USER_DATA_DIR_NAME)
}

/** Default folder for saved screenshots: `<Pictures>/<product name>`. */
export function screenshotsDir(picturesRoot: string): string {
  return join(picturesRoot, PRODUCT_NAME)
}
 ...
 */
export function applyAppIdentity(app: AppIdentityTarget): string {
  ...
  const dir = userDataDir(app.getPath('appData'))
