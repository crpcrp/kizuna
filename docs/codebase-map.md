...
| Jimaku subtitle discovery | `shared/jimaku.ts` (normalized entries, files, error results, and credential status) | `services/jimaku/client.ts`, `services/jimaku/settings.ts` (fixed-origin API client and encrypted credential owner); `jimakuSettingsBridge.ts` |
