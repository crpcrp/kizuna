...
| Knowledge tracking and JLPT coverage/export | `state/useKnowledgeOptions.ts`, `state/useVocabularyCaches.ts`, subtitle components, `state/knowledgeActions.ts`, knowledge options, `shared/knowledge.ts`, `shared/jlptCoverage.ts`, `shared/jlptExport.ts` | `knowledgeBridge.ts`, `services/knowledge/`, `services/jlpt/` |
