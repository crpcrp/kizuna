...
| Jimaku subtitle discovery | `shared/jimaku.ts` (normalized entries, files, session results, prepared selections, error results, credential status, and folder hints); `shared/jimakuMatching.ts` (local identity parsing and deterministic file ranking); `state/jimakuController.ts` (guarded search, preparation, replacement, revert, and remembered-title state); `components/JimakuSubtitlesDialog.tsx` and `JimakuSubtitlesDialog.css` (presentation-only search, remembered-title controls, and file chooser); `components/NoJapaneseSubtitlePrompt.tsx` and its CSS (post-restore discovery prompt) | `services/jimaku/client.ts`, `services/jimaku/settings.ts` (fixed-origin API client, encrypted credential, and folder-hint owner), `services/jimaku/downloadStore.ts` (bounded direct/package storage, deduplication, and cleanup), `services/jimaku/archive.ts` (ZIP inspection and selected-member extraction), `services/jimaku/service.ts` (sender-scoped search/download sessions); `jimakuSettingsBridge.ts`, `jimakuBridge.ts` |
