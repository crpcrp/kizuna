...
import { createJlptBackfillService } from '@src/main/services/anki/jlptBackfillService'
...
function serviceFor(notes: AnkiNoteInfo[], classifier: JlptClassifier = { levelFor: () => 'N5' }) {
  ...
  const service = createJlptBackfillService({
    settings,
    createClient: () => createAnkiClient({ url: anki.url, fetch: anki.fetch }),
    jlptClassifier: classifier,
    createToken: () => 'backfill-token'
  })
  return { anki, service, settings }
}

describe('createJlptBackfillService', () => {
