...
| Jimaku subtitle discovery | `shared/jimaku.ts` (normalized entries, files, session results, prepared selections, error results, and credential status); `shared/jimakuMatching.ts` (local identity parsing and deterministic file ranking); `state/jimakuController.ts` (guarded search, preparation, replacement, and revert state); `components/JimakuSubtitlesDialog.tsx` and `JimakuSubtitlesDialog.css` (presentation-only search and file chooser) | `services/jimaku/client.ts`, `services/jimaku/settings.ts` (fixed-origin API client and encrypted credential owner), `services/jimaku/downloadStore.ts` (bounded direct/package storage and deduplication), `services/jimaku/archive.ts` (ZIP inspection and selected-member extraction), `services/jimaku/service.ts` (sender-scoped search/download sessions); `jimakuSettingsBridge.ts`, `jimakuBridge.ts` |
